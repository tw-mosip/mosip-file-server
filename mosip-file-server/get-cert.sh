...
#get secret
clientsecret_env=$(curl $spring_config_url_env/*/default/$spring_config_label_env/registration-processor-mz.properties | sed -n '/token.request.secretKey=/,/ /p' | cut -d '#' -f1 |  sed 's/.*secretKey=//; s/$\n.*//' | awk 'NR==1{print $1}')
